...
{
	viewStack.popTo(viewStack.viewIdx(UI_TEXT("System Actions")));
}
...
{
	app().showUI();
	if(!viewStack.contains(UI_TEXT("System Actions")))
	{
...
void EmuViewController::onSystemClosed()
{
	...
	if(int idx = viewStack.viewIdx(UI_TEXT("System Actions"));
		idx > 0)
	{
